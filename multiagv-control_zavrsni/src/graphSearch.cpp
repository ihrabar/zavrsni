...
// ako je segment izvediv vraća se njegova cijena, u suprotnom se vraća vrijednost 100000
double cost(int x, int y, int ei, int ej, vector<vector<pair<int, int> > > *cm, const vector<vector<double> >& map, vector<pair<vector<int>, double> > *g, int node_dist)
{
    double rez = 0.0;
    ...
    if(!rem)	// ako se ne radi o removingu, umjesto Dijkstre se koristi A* algoritam (dodaje se heuristična komponenta u cijenu)
    {
...
void graphSearch(matvar_t * varVars, matvar_t * varCost, const vector<vector<double> >& vMap, int start[], int finish[], bool removing, vector<int> wrX, vector<int> wrY, double** M, int sizeM[], float mapResolution, int nodeDist)
{
	...
	{
		// "wrX" i "wrY" sadrže zabranjene čvorove grafa
		// tj. čvorove koji leže na putanji vozila koje je zatražilo removing
		// koordinate zabranjenih čvorova upisuju se u wrongx i wrongy vektore
		for (unsigned int i = 0; i < wrX.size(); i++)
		{
		    wrongx.push_back(wrX[i]);
		    wrongy.push_back(wrY[i]);
		}
		 
		// Sljedeće dvije naredbe su dodane naknadno da se izbjegne
		// zaglavljenje u nekim situacijama kod removinga  
		wrongx.push_back(xs);
		wrongy.push_back(ys);
	...
	while(true)
	{
	    ...
	    // publishanje posjećenih čvorova (za prikaz u RViz-u)
		/*a.x = v[0] * nodeDist * mapResolution;
		a.y = v[1] * nodeDist * mapResolution;
		a.z = 0.0;
	    ...
	    {
	        founded = true;
	        // provjera nalazi li se trenutni čvor u grupi zabranjenih čvorova
	        for (unsigned int i = 0; i < wrongx.size(); i++)
	        {
	            if (v[0] == wrongx[i]  &&  v[1] == wrongy[i])
	            {
	            	// trenutni čvor nalazi se u grupi zabranjenih čvorova
	                founded = false;	
	                break;
	            }
	        }
	        ...
	        if (founded)	
	        {	
				// ako se trenutni čvor NE nalazi se u grupi zabranjenih čvorova
	        	// za koordinate ciljnog čvora uzimaju se koord. trenutnog čvora
	        	// (removing u najbliži slobodni čvor)
	            xf = v[0]; yf = v[1]; fif = v[2];	
	            break;	// prekida se proces daljnjeg pretraživanja grafa
	        }
	    }
	    else if (v[0] == xf  &&  v[1] == yf  &&  v[2] == fif) // ako su koord. tenutnog čvora jednake koordinatama ciljnog čvora..
	    {
	    ...
	    for (unsigned int i = 0; i < graph[v[2]].size(); i++)	// graph[v[2]] sadrži podatke o svim susjednim čvorovima u koje se može doći iz trenutnog čvora "v" uz početnu orijentaciju v[2] 
	    {
	        ...
	        if (hm.find(new_v) == hm.end())	// ako "new_v" još nije dodan u mapu "hm"..
	        {
	            ...
	            new_c = cost(v[0], v[1], v[2], i, cost_map, vMap, graph, nodeDist);	// provjera izvedivosti i određivanje cijene segmenta
		            	
	            if (new_c < 99999.0)	// ako je segment izvediv
	            {
	        ...
	        else 
	        {
	            if (hm[new_v] != -1)	
	            {
	                ...
	                if (c + cost(v[0], v[1], v[2], i, cost_map, vMap, graph, nodeDist) < heap[hm[new_v]].second)
	                {
	                    ...
	                    heap[hm[new_v]].second = c + cost(v[0], v[1], v[2], i, cost_map, vMap, graph, nodeDist); // upisuje se nova cijena do čvora "new_v"
	                    hm_sol[new_v] = make_pair(v[2], i);	// pamti se orijentacija u prethodnom čvoru i index segmenta do čvora "new_v"  
	    ...
	    for (unsigned int i = 0; i < graph[(v[2] + 8) % 16].size(); i++)	// graph[(v[2] + 8) % 16] sadrži podatke o svim susjednim čvorovima u koje se može doći iz trenutnog čvora "v" kretanjem unatrag 
	    {
	        ...
	        {
	            //heap.push_back(make_pair(new_v, c + graph[v[2]][i].second + pen * abs(new_v[2] - v[2])));
	            new_c = 2 * cost(v[0], v[1], (v[2] + 8) % 16, i, cost_map, vMap, graph, nodeDist);	// provjera izvedivosti i određivanje cijene segmenta
	            if (new_c < 99999.0)	// ako je segment izvediv
	            {
	                heap.push_back(make_pair(new_v, c + new_c));	// čvor se dodaje na kraj stabla zajedno s pripadajućom ukupnom cijenom
	                hm[new_v] = heap.size() - 1;	// pamti se index čvora u stablu
	        ...
	        else 
	        {
	            ...
	            {
	                // ako je cijena dolaska u susjedni čvor "new_v" manja od prehodne cijene za taj čvor..
	                if (c + 2*cost(v[0], v[1], (v[2] + 8) % 16, i, cost_map, vMap, graph, nodeDist) < heap[hm[new_v]].second)
	                {
	                    if(heap[hm[new_v]].first[0] != new_v[0] ||  heap[hm[new_v]].first[1] != new_v[1] || heap[hm[new_v]].first[2] != new_v[2])
	                        fprintf(stderr, "Error in graph search!\n");

	                    //heap[hm[new_v]].second = c + graph[v[2]][i].second + pen * (abs(new_v[2] - v[2]));
	                    heap[hm[new_v]].second = c + 2*cost(v[0], v[1], (v[2] + 8)%16, i, cost_map, vMap, graph, nodeDist); // upisuje se nova cijena do čvora "new_v"
	                    hm_sol[new_v] = make_pair((v[2] + 8) % 16 + 16, i);	// pamti se orijentacija u prethodnom čvoru i index segmenta do čvora "new_v"
	                    bubble_up(hm[new_v], &heap, &hm);	// modificira se mjesto "new_v" čvora u stablu tako da stablo ponovno bude sortirano
	                }
	            }
